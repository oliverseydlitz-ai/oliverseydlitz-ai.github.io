...
| R37 | Precached shell paths (`ASSETS`) are stale-while-revalidate: a cached copy is returned instantly and a background fetch refreshes the cache for next time, instead of paying network-first-with-a-3s-timeout for a file already sitting in the cache. Anything outside the shell, or a shell path with nothing cached yet, still gets R7's network-first-with-timeout unchanged. `registerServiceWorker()` (extracted, exported to tests) now calls `reg.update()` on `visibilitychange` when the tab becomes visible, so a tab left open for days checks for a new worker instead of never asking again. `service-worker.js` extended (old sw.js/app.js fail 3 of the new checks) | 25 Sep |
| V41 | Settings sections were built two ways — a `.settings-section` card for most, a bare `<h2>` + unstyled `.settings-group` for Launch Monitor and How You Practise — and there were two toggle idioms (the dark-mode knob switch vs a bare "✓" in `.pref-toggle`, which had no CSS distinguishing on/off beyond an inline colour). Both sections now use the same `.settings-section` shell; every on/off row (View Preferences ×5, "keep sessions on this device") uses the renamed, reusable `.switch`/`.switch-knob` component `applyTheme()` already used for dark mode — `paintPref`/`paintKeepLocal` now do the same `classList.toggle('on', …)` instead of hand-writing textContent and an inline colour. Data & Export's 13 mixed rows are split: the five report launchers (Your numbers / Where you sit / What you can work on / Club by club / How you practised) moved to their own "Reports" section, leaving Data & Export as data actions only. New suite `settings-and-polish.js` (old markup/CSS fail 6 of its V41 checks) | 25 Sep |
| V46 | The printed yardage card's "Personal Bests" heading printed with its grid already hidden underneath it — an orphan heading — because only `#recordsGrid` was in the print hide-list, not its wrapping `.section-block`; that block now also carries `.no-print`. The "Sessions: …" caveat printed up to three times: `#yardagePrintHead`'s own purpose-built copy, plus `#yardageConditions`'s near-identical sentence and `#yardageLegend`'s two methodology notes, neither of which the print stylesheet hid. Both are now added to the print hide-list, so exactly one caveat prints, the one built for paper | 25 Sep |
| V47 | `.section-title` is a flex row, so a title with a `.section-sub` (Short Game, Quiet Eye, Strike Quality, Ball Flight, Benchmarking, Shot Log, On the Course) laid out as two flex items that don't shrink below their own content — at 393px "Short Game" wrapped in its own narrow column beside "putting & chipping, and what the trials actually found" wrapping in its own column beside it, instead of one heading over one line of subtitle text. `.section-title:has(.section-sub)` now stacks to a column under 480px. Verified against the real defect: the old CSS produces exactly that two-column split in a real Chromium render. EXTENDS V14 | 25 Sep |
| V48 | Three different close-button styles (`.btn-icon` on most modals, the range card's own circular `.rc-x`, and FirstRun's completely unstyled `.modal-close`) collapse to one: both `.rc-x` buttons and FirstRun's close button now carry `.btn-icon`, and `.rc-x` keeps only its absolute position. The range-card progress dots were `.rc-dot { border-radius: 50% }` — the last circle in a stylesheet with zero border-radius everywhere else — and their whole clickable area was the 9.6px visible dot, well under any touch-target minimum; the dot is now drawn on `::after` at the same visual size inside a 24px square button. `settings-and-polish.js` | 25 Sep |
| V49 | The Data & Rights modal used the sessions-list icon (three horizontal lines, reads as a hamburger menu) to represent "data", and double-padded its body (`padding:1.2rem` on an inner div sitting inside `.modal`'s own `padding: var(--s5)`) — now a lock icon and no redundant padding. Every one of the ten `Features.achievements()` definitions hardcoded `icon:'star'`, so all ten looked identical once unlocked regardless of what they measured; each now carries its own icon from the sprite. `#achHeadCount` ("3 of 10 unlocked") sat inline after the modal's `<h2>` and could break onto its own line as an orphan "unlocked" at modal width; it is now `display: block`, its own line always. Text this touches — the "48 hours" / "30 days" GDPR promises — was left exactly as written, per Oliver's standing call | 25 Sep |
| R33 (part 2) | The ~197px (200% zoom, WCAG 1.4.10) leftover: four named components, each a missing wrap or a fixed minimum width. `.dash-sub-title` (heatmap legend) had no `flex-wrap`, so it spilled out of `.dash-heatmap-wrap`. `.score-breakdown`'s flat `min-width: 112px` beside the fixed-size score ring left neither able to give; `.score-banner-content` now wraps and stacks them. `.sg-setup` (short game's setup fields) was a grid without `minmax(0, 1fr)` — the same bug `.form-row` already had, fixed the same way, with the same 380px single-column fallback. `.drill-row-head` (the name + a state/tier badge that must not itself wrap) had no `flex-wrap`. `render-scan.js`'s new ZOOM200 check verifies each by name against the viewport — not the page's whole `scrollWidth`, which would also catch unrelated, not-yet-fixed overflow on the same views (the focus/streak card, achievements, session cards, the dispersion stat grid — found beside it, not fixed) and bury a regression here inside that noise. Old CSS fails all 5 checks (`.sg-tier` counted separately from `.sg-setup`/`#sgLie`) | 25 Sep |
| R27 (part 2) | The rest of it: the nav is static markup, painted before `app.js` (behind ~300 KB of undeferred vendor scripts) has necessarily even finished downloading, and a nav that looks tappable and does nothing reads as broken. `<html class="booting">` ships in the markup (no script needed to disable it); the stylesheet gives `.bottom-nav`/`.top-nav` `pointer-events: none` and dims them while it is present. `app.js` removes the class the instant its click delegation is registered — before `Auth.init()`/`LocalDB.hydrate()`, both awaited later in `init()` with their own network dependency, so the nav is never gated on those. New suite `boot-nav.js` wraps `Auth.init()` and asserts `booting` is already gone when it fires; verified live in Chromium with `app.js`'s response delayed 2.5s (nav visibly dimmed and inert mid-load, normal once boot completes, both themes). Old markup/CSS fail 4 of 5 checks | 25 Sep |
| R36 | Measured, not shipped: **CLAUDE.md's "Performance Notes" has the full writeup.** `defer` on the five blocking scripts, on the audit's own profile (1.6 Mbps / 400ms RTT / 4× CPU), five runs each: the nav paints at ~1.03–1.06s either way (its markup precedes the scripts, so blocking vs. deferred parsing never touches it), and the nav becoming USABLE (R27's `booting` marker) averaged 8462ms blocking vs. 8446ms deferred — 16ms apart on an 8.4s load, inside the ~70ms run-to-run noise, despite confirming `defer` genuinely did make all five requests fire in parallel. The connection is bandwidth-bound (~1.2 MB, mostly unminified `app.js` and Chart.js/Supabase, over a capped pipe), not latency-bound, so parallel fetching doesn't reduce total transfer time. Left as plain blocking scripts; a real fix needs less payload, not a different loading order | 25 Sep |
