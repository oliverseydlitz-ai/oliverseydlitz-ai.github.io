...
Access to account data is restricted at the database level by row-level security policies scoped to the authenticated account, which are enforced by the database rather than by application code. Anonymous database roles hold no privileges over account data. A row size and row count ceiling is applied to limit the impact of misuse. The Service applies a restrictive Content-Security-Policy and escapes user-supplied text before rendering it.
