...
// ────────────────────────────────────────────────────────────────
// Analytics — Shot analysis helpers
// ────────────────────────────────────────────────────────────────
const Analytics = (() => {
  function shotTendency(shots) {
    const byClub = {};
    shots.forEach(s => {
      if (!byClub[s.clubType]) byClub[s.clubType] = [];
      byClub[s.clubType].push(facePath(s));
    });
    return Object.entries(byClub).map(([club, paths]) => {
      const fades = paths.filter(p => p > 0.5).length;
      const draws = paths.filter(p => p < -0.5).length;
      const tendency = fades > draws ? 'fade' : draws > fades ? 'draw' : 'straight';
      return { club, tendency, fades, draws, count: paths.length };
    }).sort((a, b) => clubOrder(a.club) - clubOrder(b.club));
  }

  function bestWorstShots(shots, limit = 5) {
    const sorted = [...shots].sort((a, b) => (b.carryDistance || 0) - (a.carryDistance || 0));
    return {
  ...
  function smashFactorRanking(shots, limit = 5) {
    return [...shots]
      .filter(s => s.smashFactor)
      .sort((a, b) => (b.smashFactor || 0) - (a.smashFactor || 0))
      .slice(0, limit);
  }

  function swingSpeedConsistency(shots) {
    const byClub = {};
    shots.forEach(s => {
      if (!byClub[s.clubType]) byClub[s.clubType] = [];
      if (s.ballSpeed) byClub[s.clubType].push(s.ballSpeed);
    });
    return Object.entries(byClub)
      .map(([club, speeds]) => {
        ...
        const consistency = Math.max(0, 100 - cv * 2.5);
        return { club, consistency: Math.round(consistency), sd, count: speeds.length };
      })
      .sort((a, b) => clubOrder(a.club) - clubOrder(b.club));
  }
  ...
  function personalBests(sessions) {
    const all = sessions.flatMap(s => s.shots);
    if (!all.length) return [];
    const carries = all.map(s => s.carryDistance).filter(v => v);
    const speeds = all.map(s => s.ballSpeed).filter(v => v);
    const smashes = all.map(s => s.smashFactor).filter(v => v);
    const bests = [];
    if (carries.length) {
      const max = Math.max(...carries);
      const shot = all.find(s => s.carryDistance === max);
      bests.push({ label: 'Longest Carry', value: Math.round(max), club: clubLabel(shot?.clubType) });
    }
    if (speeds.length) {
      const max = Math.max(...speeds);
      const shot = all.find(s => s.ballSpeed === max);
      bests.push({ label: 'Top Ball Speed', value: Math.round(max), club: clubLabel(shot?.clubType) });
    }
    if (smashes.length) {
      const max = Math.max(...smashes);
      const shot = all.find(s => s.smashFactor === max);
      bests.push({ label: 'Best Smash Factor', value: fmt(max, 2), club: clubLabel(shot?.clubType) });
    }
    return bests;
  }

  return { shotTendency, bestWorstShots, smashFactorRanking, swingSpeedConsistency, personalBests };
})();
...
const UI = (() => {
  ...
  function renderAnalytics(shots) {
    if (!shots.length) return;
    const tendencies = Analytics.shotTendency(shots);
    const smashRank = Analytics.smashFactorRanking(shots, 5);
    const consistency = Analytics.swingSpeedConsistency(shots);
    const { best, worst } = Analytics.bestWorstShots(shots, 5);

    const tendGrid = document.getElementById('tendenciesGrid');
    if (tendGrid) {
      tendGrid.innerHTML = tendencies.map(t => `
        <div class="tendency-card" style="--club-color: ${clubColor(t.club)}">
          <div class="tc-club">${clubLabel(t.club)}</div>
          <div class="tc-tend" style="color: ${t.tendency === 'fade' ? '#f59e0b' : t.tendency === 'draw' ? '#3b82f6' : '#10b981'}">
            ${t.tendency === 'fade' ? '→ Fade' : t.tendency === 'draw' ? '← Draw' : '↓ Straight'}
          </div>
          <div class="tc-counts">${t.fades}f / ${t.draws}d</div>
        </div>
      `).join('');
    }

    const smashList = document.getElementById('smashRankList');
    if (smashList) {
      smashList.innerHTML = smashRank.map((s, i) => `
        <div class="sl-item">
          <div class="sl-rank">#${i + 1}</div>
          <div class="sl-club" style="color: ${clubColor(s.clubType)}">${clubLabel(s.clubType)}</div>
          <div class="sl-value">${fmt(s.smashFactor, 2)}</div>
        </div>
      `).join('');
    }

    const consistGrid = document.getElementById('consistencyGrid');
    if (consistGrid) {
      consistGrid.innerHTML = consistency.map(c => `
        <div class="consistency-card">
          <div class="cc-club">${clubLabel(c.club)}</div>
          <div class="cc-bar">
            <div class="cc-fill" style="width: ${c.consistency}%; background: ${c.consistency >= 80 ? '#10b981' : c.consistency >= 60 ? '#f59e0b' : '#ef4444'}"></div>
          </div>
          <div class="cc-score">${c.consistency}/100</div>
        </div>
      `).join('');
    }
    ...
    if (bwGrid) {
      bwGrid.innerHTML = `
        <div class="bw-col">
          <div class="bw-title">Best Shots</div>
          ${best.map(s => `
            <div class="bw-shot">
              <span class="bw-club" style="color: ${clubColor(s.clubType)}">${clubLabel(s.clubType)}</span>
              <span class="bw-dist">${Math.round(s.carryDistance || 0)}y</span>
            </div>
          `).join('')}
        </div>
        <div class="bw-col">
          <div class="bw-title">Worst Shots</div>
          ${worst.map(s => `
            <div class="bw-shot">
              <span class="bw-club" style="color: ${clubColor(s.clubType)}">${clubLabel(s.clubType)}</span>
              <span class="bw-dist">${Math.round(s.carryDistance || 0)}y</span>
